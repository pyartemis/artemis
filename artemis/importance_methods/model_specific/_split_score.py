...
class SplitScoreImportance(VariableImportanceMethod):
    ...
    def importance(self,
        ...
        self.full_result = _calculate_all_variable_importance(trees_df, features, selected_metric)
...
def _calculate_all_variable_importance(trees_df: pd.DataFrame, features: List[str] = None, selected_metric:  SplitScoreMetric = SplitScoreMetric.SUM_GAIN): 
    ...
    basic_metrics = calculate_basic_metrics(trees_df)
    root_metrics = calculate_root_metrics(trees_df)
    mean_weighted_depth = calculate_mean_weighted_depth_metric(trees_df)

    importance_full_result = basic_metrics.join(root_metrics)
    importance_full_result = pd.concat([importance_full_result, mean_weighted_depth], axis=1).reset_index()
    
    return importance_full_result.sort_values(selected_metric, ascending=False) 

def calculate_basic_metrics(trees_df: pd.DataFrame):
...
def calculate_root_metrics(trees_df: pd.DataFrame):
...
def _select_metric(importance_full_result: pd.DataFrame, selected_metric: SplitScoreMetric):
    ...
    return variable_importance.sort_values(by="Value", ascending=False, ignore_index=True)

def _calculate_depth(trees_df: pd.DataFrame, show_progress: bool = False):
    if show_progress:
        tqdm.pandas()
        trees_df = trees_df.groupby("tree", group_keys=True).progress_apply(_calculate_depth_for_one_tree).reset_index(drop=True)
    else:
        trees_df = trees_df.groupby("tree", group_keys=True).apply(_calculate_depth_for_one_tree).reset_index(drop=True)
    return trees_df
