...
class GreenwellMethod(PartialDependenceBasedMethod):
    ...
    def _ovo(self, predict_function, model, X_sampled: pd.DataFrame, show_progress: bool, batchsize: int):
        full_pd = self._calculate_pdp_with_batch(model, predict_function, X_sampled, show_progress, batchsize)
        value_pairs = []
    ...
    def _calculate_pdp_with_batch(self, model, predict_function, X_sampled, show_progress, batchsize):
        ...
        for i, j in tqdm(self.pairs, desc=ProgressInfoLog.CALC_OVO, disable=not show_progress):
